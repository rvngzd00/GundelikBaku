...
export async function publicRoutes(app:FastifyInstance):Promise<void>{
  app.get('/home',async(_request,reply)=>{const s=await store();const[menus,categories,products,campaigns,posts,journal]=await Promise.all([
    ...
    pool.query(`SELECT id,parent_id,name,slug,description,position FROM categories WHERE store_id=$1 AND status='active' ORDER BY position,name LIMIT 100`,[s.id]),
    pool.query(`SELECT pl.id,pl.title,pl.slug,pl.short_description,pl.description,pl.price,pl.compare_at_price,pl.currency,pl.seo_title,pl.seo_description,p.sku,p.attributes,p.product_type,v.display_name AS vendor_name,b.name AS brand_name,ma.public_url AS image_url,ma.alt_text,(SELECT pv.id FROM product_variants pv WHERE pv.product_id=p.id AND pv.status='active' ORDER BY pv.created_at LIMIT 1) AS variant_id,coalesce((SELECT sum(i.quantity-i.reserved) FROM product_variants pv JOIN inventory i ON i.variant_id=pv.id WHERE pv.product_id=p.id),0)::int AS stock FROM product_listings pl JOIN products p ON p.id=pl.product_id JOIN vendors v ON v.id=p.vendor_id LEFT JOIN brands b ON b.id=p.brand_id LEFT JOIN product_media pm ON pm.product_id=p.id AND pm.is_primary LEFT JOIN media_assets ma ON ma.id=pm.media_asset_id WHERE pl.store_id=$1 AND pl.locale=$2 AND pl.status='published' AND p.deleted_at IS NULL ORDER BY pl.published_at DESC LIMIT 20`,[s.id,s.locale]),
    ...
    pool.query(`SELECT id,title,slug,excerpt,post_type,published_at,seo_title,seo_description FROM posts WHERE store_id=$1 AND locale=$2 AND status='published' AND deleted_at IS NULL ORDER BY published_at DESC LIMIT 6`,[s.id,s.locale]),
  ...
  ]);reply.header('Cache-Control','public, max-age=60, stale-while-revalidate=300');return{data:{store:s,menus:menus.rows,categories:categories.rows,products:products.rows,campaigns:campaigns.rows,posts:posts.rows,journal:journal.rows[0]??null}};});
