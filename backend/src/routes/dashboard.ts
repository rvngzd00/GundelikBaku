...
export async function dashboardRoutes(app: FastifyInstance): Promise<void> {
  app.get('/', { preHandler: app.requirePermission('dashboard.read') }, async (request) => {
    ...
    const vendorFilter = actor.isSuperAdmin || actor.vendorIds.length === 0
    ...
    const [orders, products, vendors, qr] = await Promise.all([
      ...
      pool.query(`
        ...
        ${actor.isSuperAdmin || actor.vendorIds.length === 0 ? '' : 'AND p.vendor_id = ANY($1::uuid[])'}
      `, actor.isSuperAdmin || actor.vendorIds.length === 0 ? [] : [actor.vendorIds]),
      actor.permissions.has('vendors.read')
        ? pool.query(`SELECT count(*)::int AS total, count(*) FILTER (WHERE status = 'pending')::int AS pending FROM vendors WHERE deleted_at IS NULL`)
        : Promise.resolve({ rows: [{ total: 0, pending: 0 }] }),
      pool.query(`
        SELECT count(*)::int AS codes, coalesce(sum(scan_count), 0)::bigint AS scans
        FROM qr_codes
        WHERE status = 'active'
        ${actor.isSuperAdmin || actor.vendorIds.length === 0 ? '' : 'AND vendor_id = ANY($1::uuid[])'}
      `, actor.isSuperAdmin || actor.vendorIds.length === 0 ? [] : [actor.vendorIds])
    ]);
